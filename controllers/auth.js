...
module.exports = {
  async CreateUser(req, res) {
    ...
    return bcrypt.hash(value.password, 10, (err, hash) => {
      ...
      User.create(body)
        ...
        .catch(err => {
          res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({message: 'Error occurred', err});
        })
    });
  },
  ...
  async LoginUser(req, res) {
    ...
    await User.findOne({username: Helpers.firstLetterUpper(req.body.username)}).then(user => {
      if (!user) {
        return res
